...
# Creates a minimal SliTaz Linux image for Hyper-V
#
# Requirements:
...
SLITAZ_ISO_URL="${3:-https://mirror1.slitaz.org/iso/rolling/slitaz-rolling-core64.iso}"
...
# Step 1: Download SliTaz
echo "[1/6] Downloading SliTaz Linux..."
if [ ! -f slitaz.iso ]; then
    wget -q --show-progress "$SLITAZ_ISO_URL" -O slitaz.iso
fi

# Step 2: Extract kernel and initrd
echo "[2/6] Extracting kernel and initrd..."
mkdir -p iso-mount
7z x slitaz.iso -oiso-mount/ 2>/dev/null || {
    # Fallback: use mount if 7z not available
    mkdir -p iso-mount
    mount -o loop slitaz.iso iso-mount 2>/dev/null || {
        echo "Error: Cannot extract ISO. Install p7zip or use WSL."
        exit 1
    }
}

cp iso-mount/boot/bzImage "$OUTPUT_DIR/kernel"
cp iso-mount/boot/rootfs4.gz "$OUTPUT_DIR/initrd"
...
echo "  Git, bash, curl: included in SliTaz base"
